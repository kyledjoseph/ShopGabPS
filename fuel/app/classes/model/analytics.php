...
class Model_Analytics extends \Model
{
	...
	public function get_csv($type)
	{
		...
		if ($type == 'products')
		{
			...
			{
				$csv.= "{$quest_product->id},{$quest_product->quest_id},{$quest_product->quest->name},{$quest_product->quest->user_id},{$quest_product->quest->user->display_name()},{$quest_product->product_id},{$quest_product->description},{$quest_product->total_likes},{$quest_product->total_dislikes},{$quest_product->added_by},{$quest_product->user->display_name()},{$quest_product->created_at},{$quest_product->updated_at}";
			}
			return $csv;
		}
